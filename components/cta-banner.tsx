      ...
      <div className="overflow-hidden rounded-[2rem] border border-teal-200 bg-[linear-gradient(135deg,#0f172a_0%,#134e4a_100%)] px-5 py-8 text-white sm:px-8 lg:px-12 lg:py-10">
        <div className="flex flex-col gap-8 lg:flex-row lg:items-center lg:justify-between">
          ...
          <div className="flex w-full flex-col gap-3 sm:w-auto sm:flex-row sm:flex-wrap sm:gap-4">
            <Button href={primary.href} className="w-full bg-white text-[#102129] hover:bg-[#dce8eb] sm:w-auto">
              {primary.label}
              <ArrowRight className="size-4" strokeWidth={2} aria-hidden="true" />
            </Button>
            {secondary ? <Button href={secondary.href} variant="secondary" className="w-full border-white/20 bg-white/8 text-white hover:border-white/45 hover:bg-white/12 hover:text-white sm:w-auto">{secondary.label}</Button> : null}
          </div>
        </div>
      </div>
    </section>
